...
Контекст:
	...
	И проверена установка "i_view32"
	И проверена установка "python"

Сценарий: Обновление сервисных баз из исходников
	Дано: я склонировал "https://github.com/silverbulleters/vanessa-behavior.git" ветку "feature/build-from-one-step" во временный каталог
	Когда: я вызываю команду системы "oscript ./tools/contribution.os update-conf" в каталоге проекта
	Тогда: создаются сервисные конфигурации без ошибок

Сценарий: Запуск самопроверки после доработки контрибьютором
	Когда: существует конфигурационный файл самопроверки для каждой версии 1С
	И я вызываю команду системы "oscript ./tools/contribution.os check-my-build" в каталоге проекта
